...
def record_pending(name, url, error):
    """记录 SSL 证书失败的域名到待复核列表"""
    pending = []
    try:
...
def _urlopen_with_ssl_skip(url, trusted_domains):
    """如果域名在 trusted 列表且遇到 SSL 错误，跳过验证重试"""
    req = urllib.request.Request(url, headers={"User-Agent": UA})
    try:
        return urllib.request.urlopen(req, timeout=TIMEOUT)
    except Exception as e:
        err = str(e)
        if "SSL" in err or "CERTIFICATE" in err or "certificate verify failed" in err:
            if any(url.startswith(t) for t in trusted_domains):
...
def fetch_rss_direct(url, source_name):
    ...
    req = urllib.request.Request(url, headers={"User-Agent": UA})
    with _urlopen_with_ssl_skip(url, load_trusted()) as resp:
        xml_data = resp.read()
...
def fetch_web_list(url, source_name):
    ...
    for idx, (link_url, title) in enumerate(article_links):
        ...
        try:
            ...
            with _urlopen_with_ssl_skip(link_url, load_trusted()) as resp:
...
def collect_monitors():
    ...
    for m in mons:
        ...
        try:
            ...
            err = str(e)
            if "SSL" in err or "CERTIFICATE" in err or "certificate verify failed" in err:
                if any(url.startswith(t) for t in load_trusted()):
                    print("[warn] monitor %s SSL error but trusted, skipping: %s" % (name, err), file=sys.stderr)
                else:
                    record_pending(name, url, err)
                    print("[warn] monitor %s SSL failed (pending review): %s" % (name, err), file=sys.stderr)
            else:
                print("[warn] monitor %s failed: %s" % (name, err), file=sys.stderr)
    return all_items
